...
async function loadDealsDashboard() {
  ...
  try {
    const response = await fetch(
      `/api/deals?startDate=${dates.startDate}&endDate=${dates.endDate}`
    );
    ...
    loadSuma(dates.startDate, dates.endDate);
...
async function loadPlanas(startDate, endDate) {
  try {
    const response = await fetch(`/api/deals/planas?startDate=${startDate}&endDate=${endDate}`);
    const data = await response.json();
...
async function loadDealsPlanas(startDate, endDate) {
  try {
    const response = await fetch(`/api/deals/planas-kiekis?startDate=${startDate}&endDate=${endDate}`);
    const data = await response.json();
    if (data.success && data.planas !== null) {
...
async function loadSuma(startDate, endDate) {
  try {
    const response = await fetch(`/api/deals/suma?startDate=${startDate}&endDate=${endDate}`);
    const data = await response.json();
    if (data.success) {
      document.getElementById('statSumaPlanas').textContent = formatCurrency(data.suma);
...
async function loadDailyChart(startDate, endDate) {
  try {
    const response = await fetch(`/api/deals/daily?startDate=${startDate}&endDate=${endDate}`);
    const data = await response.json();
    if (data.success) {
...
function updateCampaignAnalysis(stats, campaignStats) {
  ...
  if (avgCPLEl) {
    ...
    if (totalKiekis > 0) {
      avgCPLEl.style.color = avgCPL > CPL_PLAN ? '#f46565' : '';
    }
  }

  const cplPlanEl = document.getElementById('statAvgCPLPlan');
  if (cplPlanEl) {
    cplPlanEl.textContent = formatCurrency(CPL_PLAN);
  }
...
function updateCplProgressBar(avgCPL) {
  ...
  if (!bar) return;
  const percent = CPL_PLAN > 0 ? Math.min((avgCPL / CPL_PLAN) * 100, 100) : 0;
  bar.style.width = percent + '%';
  bar.style.backgroundColor = avgCPL > CPL_PLAN ? '#f46565' : '#3AB493';
}
